...
public class Svm {
	...
	/*
	 * private static SvmPrintInterface svm_print_stdout = new
	 * SvmPrintInterface() { public void print(String s) { System.out.print(s);
	 * System.out.flush(); } };
	 * 
	 * private static SvmPrintInterface svm_print_string = svm_print_stdout;
	 * 
	 * static void info(String s) { svm_print_string.print(s); }
	 */
	...
	private static void sigmoid_train(int l, double[] dec_values, double[] labels, double[] probAB) {
		...
		for (iter = 0; iter < max_iter; iter++) {
			...
			if (stepsize < min_step) {
				// Svm.info("Line search fails in two-class probability
				// estimates\n");
				break;
			}
		}
		...
		if (iter >= max_iter)
			// Svm.info("Reaching maximal iterations in two-class probability
			// estimates\n");
			probAB[0] = A;
		probAB[1] = B;
	}
	...
	public static SvmModel svm_train(SvmProblem prob, SvmParameter param) {
		...
		if (param.svm_type == SvmParameter.ONE_CLASS || param.svm_type == SvmParameter.EPSILON_SVR
			...
			if (nr_class == 1)
				// Svm.info("WARNING: training data in only one class. See
				// README for details.\n")
				;
	...
	/*
	 * public static void svm_set_print_string_function(SvmPrintInterface
	 * print_func) { if (print_func == null) svm_print_string =
	 * svm_print_stdout; else svm_print_string = print_func; }
	 */
}
